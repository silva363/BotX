import { checkHelper, doCycle, doCyclePol, doTransfer, doTransferFlow, doTransferFlowPol, swapBuy } from './transactionService';
...
import { getMaticBalance, generateNewWallet, getSelectedTokenBalance2, getSigner, getTokenPrice } from '../helpers/ethersHelper';
...
const listBuyingBots: TradeBot[] = [];
const type = 'Buy';

export class TradeBotBuyService {
  ...
  async run(userId: number, uuid: string, fromFront: boolean, isNewExecution: boolean, executionId: number) {
    try {
      ...
      const isRunning = listBuyingBots.findIndex((listData) => listData.uuid === tradeBot.uuid);
  ...
  async execution(tradeBot: TradeBot, tradeBotFlow: TradeBotFlow, signer: Signer, polNeedTax: bigint) {
    try {
      ...
      const amount = getTokenRange(tradeBot.min_amount, tradeBot.max_amount, tokenDetails.pool_decimals);
      const selectedTokenBalance = await getSelectedTokenBalance2(signer, tokenDetails.pool_address);
      ...
      if (selectedTokenBalance > BigInt(0) && selectedTokenBalance > amount) {
        ...
        console.log(`[${type}] ${tradeBot.name} insuficient [${tokenDetails.pool_symbol}] balance`);
      }
      ...
      if (tradeBot.target_balance == 0 || Number(tradeBot.spent_balance) < tradeBot.target_balance) {
        ...
        await removeFromList(listBuyingBots, tradeBot.uuid);
  ...
  async stop(userId: number, uuid: string): Promise<void> {
    ...
    if (botData.strategy != 'buy') {
      throw 'This is not a buy bot';
    }

    await removeFromList(listBuyingBots, botData.uuid);
  }
  ...
  async restartSwap(executionId: number, userId: number, uuid: string, swap: Swap) {
    try {
      ...
      const oldTokenBalance = await getSelectedTokenBalance2(oldMainGhost, tokenDetails.pool_address);
      ...
      const actualTokenBalance = await getSelectedTokenBalance2(newMainGhost!, tokenDetails.pool_address);
  ...
  async startIdleSwaps(tradeBot: TradeBot, useTimer: boolean) {
    try {
      ...
      for (let index = 0; index < list.length; index++) {
        ...
        const actualTokenBalance = await getSelectedTokenBalance2(signer, tokenDetails.pool_address);
  ...
  try {
    ...
    for (let index = 0; index < lastTransactions.length; index++) {
      ...
      if (element.new_wallet_private_key) {
        ...
        const pendingTokenBalance = await getSelectedTokenBalance2(txSigner, tokenDetails.pool_address);
    ...
    const tokenBalance = await getSelectedTokenBalance2(restartSigner, tokenDetails.pool_address);
...
async function executeGhostWallets(
  ...
  try {
    ...
    if (tradeBotFlow.actual_cycle <= tradeBot.cycles) {
      ...
      const tokenBalance = await getSelectedTokenBalance2(mainGhost, tokenDetails.pool_address);
...
async function startPoolFlow(executionId: number, tradeBot: TradeBot, polBalance: bigint, tokenBalance: bigint, mainGhost: HDNodeWallet) {
  try {
    ...
    console.log(`[${type}] ${tradeBot.name} swapping [${tokenDetails.pool_symbol}] to [${tokenDetails.symbol}]...`);
    ...
    if (swapPolToTokenPool.status != 1) {
      throw `Swap [${tokenDetails.pool_symbol}] to [${tokenDetails.symbol}] fail`;
    }
...
async function startSwapFlow(executionId: number, tradeBot: TradeBot, polBalance: bigint, tokenBalance: bigint, mainGhost: HDNodeWallet, tokenDetails: AcceptedToken, swap: Swap | null = null) {
  ...
  try {
    console.log(`[${type}] ${tradeBot.name} swapping [${tokenDetails.pool_symbol}] to [${tradeBot.token_symbol}]...`);
    ...
    if (!swap) {
      const swapModel: Swap = {
        ...
        amount: ethers.formatEther(polBalance),
        token_name: tokenDetails.pool_name,
    ...
    if (tradeBot.mode == 'queue') {
      console.log(`[SWAP BUY] with token [${tokenDetails.pool_symbol}] has been placed in the queue`);
      return;
    }

    const maticUsdPrice = await getTokenPrice(mainGhost, settings.MATIC_USD_ADDRESS, 'USDT', 'Tether USD', 6, null);
    const tokenPrice = await getTokenPrice(mainGhost, tokenDetails.address, tokenDetails.symbol, tradeBot.name, tokenDetails.decimals, tokenDetails);
    ...
    if (selectedTokenPrice >= tradeBot.target_price) {
      timerCountdown(5, `[${tradeBot.strategy}] ${tradeBot.name} ${tokenDetails.pool_symbol} value is above target price`, false);
    ...
    */
    ...
    if (resSwap.status !== 1) {
      throw `[SWAP BUY] with token [${tokenDetails.pool_symbol}] and [${tradeBot.token_symbol}] fail`;
    }

    console.log(`[${type}] ${tradeBot.name} swap finished`);
    startDestinyFlow(executionId, tradeBot, mainGhost);
  } catch (error) {
    console.log(`[${type}] ${tradeBot.name} ${error}`);
...
async function startIdleSwapFlow(signer: Signer, privateKey: string, executionId: number, tradeBot: TradeBot, polBalance: bigint, tokenBalance: bigint, tokenDetails: AcceptedToken, swap: Swap) {
  ...
  try {
    ...
    startIdleDestinyFlow(executionId, tradeBot, signer, privateKey);
  } catch (error) {
    console.log(`[${type}] ${tradeBot.name} ${error}`);
...
async function startDestinyFlow(executionId: number, tradeBot: TradeBot, mainGhost: HDNodeWallet) {
  try {
    const resultTokenBalance = await getSelectedTokenBalance2(mainGhost, tradeBot.token_address);
    ...
    console.log(`[${type}] ${tradeBot.name} doing [${tradeBot.token_symbol}] airdrop...`);
...
async function startIdleDestinyFlow(executionId: number, tradeBot: TradeBot, signer: Signer, signerPrivateKey: string) {
  try {
    const resultTokenBalance = await getSelectedTokenBalance2(signer, tradeBot.token_address);
...
async function forceStop(tradeBot: TradeBot, tradeBotFlow: TradeBotFlow, executionStatus: number) {
  try {
    ...
    //removeFromList(listBuyingBots, tradeBot.uuid);
  } catch (error) {
